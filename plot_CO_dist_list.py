...
def mk_fig_histgram(array, figsize=6):
    ...
    if figsize == 6:
        fontsize = 13
    axes.set_title('Histgram of distances between carbon and oxygen under 5Å',
                   y=1.05,
                   fontsize=fontsize,)
    # 軸ラベルの設定
...
def mk_fig_histgram_under200pm(array, figsize=6):
    """
    This func() make a histgram under 2Å.
    ...
    if figsize == 6:
        fontsize = 13
    axes.set_title('Histgram of distances between carbon and oxygen under 2Å',
                   y=1.05,
                   fontsize=fontsize,)
    # 軸ラベルの設定
    axes.set_xlabel('Distance(Å)', fontsize=15)
    ...
    axes.yaxis.set_label_coords(-0.11, 0.5)  # y軸ラベルの位置を調整
    # x軸の範囲設定
    axes.set_xlim(left=0.7, right=2)
    # y軸を指数表示
    axes.ticklabel_format(style="sci",  axis="y", scilimits=(0, 0))
    # 凡例の設定
    axes.legend(loc='upper left', bbox_to_anchor=(0.01, 0.99), fontsize=fontsize-3)  # 凡例を表示
...
CO_bond_dist_1d_list_under2 = CO_bond_dist_1d_list_loaded[CO_bond_dist_1d_list_loaded <= 2]
mk_fig_histgram_under200pm(array=CO_bond_dist_1d_list_under2, figsize=args[1])
print(f"A histgram of CO-bond distances under 2Å was made \n\
